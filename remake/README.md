...
進行波・媒質の振動・波の合成・縦波の4つをまとめたフォルダです。
...
上に横波表示（媒質の変位を縦にとったグラフ）、下に実際の縦波（媒質そのものが左右に振動するようす）を並べます。同じ位置どうしが縦線でつながっているので、横波表示の形と、実際の疎密の位置の対応が読み取れます。

- 進行波：疎密が右へ進んでいく
- 定常波：節では媒質が動かず、節のところで疎密ができる
- 緑／赤球・青球：目印の玉を出す（定常波では青＝節、赤＝腹）
